...
function ProfileCard({ profile }: { profile: DirectoryProfile }) {
  ...
  return (
    <div className="group relative flex flex-col rounded-xl border border-primary-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-primary-800 dark:bg-primary-900/50">
      {/* Avatar + Name */}
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <h3 className="truncate text-sm font-semibold text-primary-800 dark:text-white">
              {profile.display_name}
            </h3>
            <DirectoryTrustBadge userId={profile.user_id} />
          </div>
          {profile.company_name && (
            <p className="mt-0.5 flex items-center gap-1 truncate text-xs text-primary-500 dark:text-primary-400">
      ...
      {/* Bio */}
      {profile.bio && (
        <p className="mt-3 line-clamp-2 text-xs text-primary-600 dark:text-primary-400">
          {profile.bio}
        </p>
      )}
      ...
      {profile.specialties.length > 0 && (
        <p className="mt-2 text-xs text-primary-500 dark:text-primary-400">
          ...
          {profile.specialties.length > 4 && ` · +${profile.specialties.length - 4}`}
        </p>
      )}
          ...
          {data.recommendations.length === 0 ? (
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {data.recommendations.map((rec) => (
                <div
                  key={rec.user_id}
                  className="rounded-lg border border-purple-200 bg-white p-3 dark:border-purple-800/50 dark:bg-primary-900/50"
                >
                  <div className="flex items-center justify-between">
                    <p className="truncate text-xs font-semibold text-primary-800 dark:text-white">
                      {rec.name}
                    </p>
                  ...
                  <ul className="mt-1.5 space-y-0.5">
                    {rec.reasons.slice(0, 2).map((reason, i) => (
                      <li
                        key={i}
                        className="text-[10px] text-primary-500 dark:text-primary-400"
                      >
                        {reason}
                      </li>
                    ))}
    ...
    return (
      <>
        <div className="mx-auto max-w-lg py-16 text-center">
          <ArrowTopRightOnSquareIcon className="mx-auto h-12 w-12 text-primary-300 dark:text-primary-600" />
          <h2 className="mt-4 text-lg font-semibold text-primary-800 dark:text-white">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="w-full sm:w-64">
            <label className="mb-1 block text-xs font-medium text-primary-500 dark:text-primary-400">
              Search
            </label>
            <div className="relative">
              <MagnifyingGlassIcon className="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-primary-400" />
              <input
                type="text"
                value={search}
        ...
        {isLoading ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="h-[220px] animate-pulse rounded-xl border border-primary-200 bg-primary-100 dark:border-primary-800 dark:bg-primary-900/50"
              />
            ))}
          </div>
        ) : profiles.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <MagnifyingGlassIcon className="h-10 w-10 text-primary-300 dark:text-primary-600" />
            <p className="mt-3 text-sm text-primary-500 dark:text-primary-400">
              {debouncedSearch || industry || location || specialty
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="inline-flex items-center gap-1 rounded-lg border border-primary-200 px-3 py-1.5 text-xs font-medium text-primary-700 hover:bg-primary-50 disabled:opacity-40 dark:border-primary-700 dark:text-primary-300 dark:hover:bg-primary-800"
            >
